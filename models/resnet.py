...
class ResNet(nn.Module):
    ...
    def _stride_dilation(dilation, mod_id, block_id):
        ...
        return s, d
    # 决定每个 residual block 的 stride（步长）和dilation

    def forward(self, x):
        outs = []
        attentions = []

        x = self.mod1(x)  # （24，64，32，32）
        #attentions.append(x)
        outs.append(x)
        ...
        x, att = self.mod3(x)  # （24，512，32，32）
        attentions.append(att)
        outs.append(x)

        x, att = self.mod4(x)  # （24，1024，32，32）
        attentions.append(att)
        outs.append(x)

        x, att = self.mod5(x)  # （24，2048，32，32）
        attentions.append(att)
        outs.append(x)
