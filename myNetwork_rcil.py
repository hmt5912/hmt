...
import inplace_abn
import models_rcil
from inplace_abn import ABN, InPlaceABN, InPlaceABNSync  # InPlaceABN一种新的批量归一化方法
...
#几种归一化的方法，iabn_sync 是同步版本，适合多卡训练，减少内存消耗，iabn 是非同步版本，适合单卡训练，内存消耗较低，但不适合分布式训练，
    # abn_sync_test (用于测试的 InPlaceABN Sync):
    body = models_rcil.__dict__[f'net_{opts.backbone}'](norm_act=norm, output_stride=opts.output_stride)  # 在resnet.py脚本上选择backbone
    # 选择backbone
    ...
    # 加载预训练权重，将权重加载到新的模型中，并处理去除与当前模型不匹配的层
    head_channels = 256
...
# 将该维度的元素的顺序倒置，[slice(None)] * x.dim() 生成一个长度为 x.dim() 的切片列表，每个维度都使用 slice(None) 来表示选择该维度的所有元素。

class IncrementalSegmentationModule(nn.Module):
# backbone用resnet，头用deeplabV3
    def __init__(
        self,
        body,
        head,
        ...
        ncm=False,
        fusion_mode="mean",
        nb_background_modes=1, # 是否将背景拆分为多个子类别
        multimodal_fusion="sum", # fusion_mode多模型输出融合方式
        use_cosine=False,
...
# classes每个元素表示 一个任务 的类别数
        use_bias = not use_cosine

        if nb_background_modes > 1: #背景有几种模式
            classes[0] -= 1 # 5-1=4
            classes = [nb_background_modes] + classes
            # 原始 classes = [5,3,4], nb_background_modes = 2，classes -> [2, 5-1=4, 3, 4]

        if only_base_weights:
            classes = [classes[0]] # 这个参数可能用于表示是否只需要使用基础权重（即仅仅使用类别的基础部分权重）

        if opts.dataset == "cityscapes_domain":
            classes = [opts.num_classes]  # 不做增量学习，直接使用全部类别数

        self.cls = nn.ModuleList([nn.Conv2d(head_channels, c, 1, bias=use_bias) for c in classes]) # 它 为每个增量任务创建一个分类头
        self.classes = classes # 每个任务类别列表
        self.head_channels = head_channels # 分类器输入通道大小
        self.tot_classes = reduce(lambda a, b: a + b, self.classes) # 总类别数累加
        self.means = None

        self.multi_modal_background = nb_background_modes > 1
        self.disable_background = disable_background # 是否忽略背景
