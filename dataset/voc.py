...
class VOCSegmentation(data.Dataset):
    ...
    def __init__(self, root, image_set='train', is_aug=True, transform=None):
        ...
        if is_aug and image_set == 'train':
            mask_dir = os.path.join(voc_root, 'SegmentationClassAug') # './data/PascalVOC12'/SegmentationClassAug
            assert os.path.exists(mask_dir), "SegmentationClassAug not found"
            split_f = os.path.join(splits_dir, 'train_aug.txt') # 这个是标签文件吗？train_aug.txt获取训练图像名
        else:
        ...
        with open(os.path.join(split_f), "r") as f:
            file_names = [x[:-1].split(' ') for x in f.readlines()]  # 划分为两个文件名

        # REMOVE FIRST SLASH OTHERWISE THE JOIN WILL start from root
        self.images = [
            (
                os.path.join(voc_root,x[0][1:]), os.path.join(voc_root, x[1][1:]) # 分别构造出两个路径，一个是原图，一个是标签
            ) for x in file_names
        ]
