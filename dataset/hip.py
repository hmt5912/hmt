...
class hipFracSegmentation(data.Dataset):
    def __init__(self, root, image_set="train", transform=None, file_list=None,
...
class hipFracSegmentationIncremental(data.Dataset):
    def __init__(
            self,
            root,
            train=True,
        ...
        if train:
            split_file = os.path.join(
                root, split_root, "hip", task_name, f"step{step}", f"client_{client_id}.txt"
            )
        else:
            split_file = os.path.join(
                root, split_root, "hip", task_name, f"step{step}", "val.txt"
            )
        ...
        roots_map = {
            "hip": "/mnt/newdisk/hmt/kt2/hip/slices",
            "lits": "/mnt/newdisk/LITS/slices",
        }
        # roots_map = {
        #     "hip": "/mnt/newdisk/hmt/kt2/hip/slices_try",
