...
class IncrementalSegmentationModule(nn.Module):
    ...
    def __init__(
        ...
        self.cls = nn.ModuleList([nn.Conv2d(head_channels, c, 1, bias=use_bias) for c in classes])  # 每个任务有一个分类头
    ...
    def align_weight(self,align_type):
        old_weight_norm = self._compute_weights_norm(self.cls[:-1], only=align_type)  # 计算旧分类器权重的范数/尺度，用来对齐新分类器

        new_weight_norm = self._compute_weights_norm(self.cls[-1:])  # 表示的是新任务的分类头，不是新类别
