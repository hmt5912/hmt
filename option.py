...
def args_parser():
    ...
    parser.add_argument("--data_root", type=str, default=r"/mnt/newdisk/hmt/kt2/amos/amos_slices_12.16/image-all-ct", help="path to Dataset")
    parser.add_argument('--dataset', type=str, default='amos', choices=['voc', 'ade', 'rib', 'amos','hip'], help="name of dataset")
    parser.add_argument("--task", type=str, default="13-2", choices=task_self1.get_task_list(),
                        help="Task to be executed (default: 4-4)")
    parser.add_argument('--incremental_method', type=str, default='OURS', help="name of method")
    parser.add_argument("--name", type=str, default='med-base',choices=['med-base', 'med-peso', 'med-dro', 'med-avg'],
                        help="name of the experiment - to append to log directory (default: Experiment)")
    parser.add_argument('--class_ratio', type=float, default=1.0, help='ratio of classes (>1) for local clients')
    parser.add_argument('--sample_ratio2', type=float, default=0.8, help='ratio of data for local clients (class>1)')
    parser.add_argument('--sample_ratio1', type=float, default=0.8, help='ratio of data for local clients (class=1)')
    parser.add_argument('--batch_size', type=int, default=12, help='size of mini-batch')
    parser.add_argument('--epochs_local', type=int, default=6, help='local epochs of each global round')
    parser.add_argument('--lr1', type=float, default=0.005, help='learning rate step=0')
    ...
    parser.add_argument('--local_clients', type=int, default=3, help='the number of selected clients in each round')
    parser.add_argument('--epochs_global', type=int, default=30, help='total number of global rounds')
    parser.add_argument('--steps_global', type=int, default=10, help='the global rounds for each step (task)')
    parser.add_argument("--seed", type=int, default=42, help="random seed (default: 2023)")
    parser.add_argument("--use_entropy_detection", action="store_true", default=True)
    parser.add_argument("--entropy_threshold", type=float, default=0.5, help="threshold for entropy detection")
    parser.add_argument("--log_dir", type=str, default='./runs/runs-amos',help='Base directory for TensorBoard logs')
    parser.add_argument("--log_name", type=str, default='amos13-2',help='Experiment name for logging')
    parser.add_argument("--dro_enable", type=str, default='True', help='true is open DRO')
    parser.add_argument("--dro_scope", type=str, default='reliable', choices=["all", "reliable", "fg_only"],
                        help='the type of DRO to use')
    parser.add_argument("--dro_tau", type=float, default=0.5, help="threshold for DRO")
    parser.add_argument("--dro_warmup_epochs", type=int, default=1, help="the warmup epochs for DRO")
    parser.add_argument("--dro_llc", type=float, default=0.1, help="the warmup epochs for DRO")
    parser.add_argument("--dro_level", type=str, default='pixel', choices=["pixel", "sample"],
                        help='the level of DRO to use')
    parser.add_argument("--resume1", action="store_true", help="resume from last_fed checkpoint")
    parser.add_argument("--reset", action="store_true", help="ignore checkpoint and start from scratch")
    ...
    parser.add_argument(
        ...
        type=str,
        default="iabn_sync",
        choices=['iabn_sync', 'iabn', 'abn', 'std', 'iabn_sync_test','bn'],
    ...
    parser.add_argument(
        "--checkpoint", type=str, default="./checkpoints/checkpoints_amos_42"
    )

    parser.add_argument(
        "--resume", type=str, default="./resumes/resumes_amos_42"
    )
    ...
    parser.add_argument("--code_directory", default="/mnt/newdisk/hmt/kt2-12.4/FISS-self-linux")
    ...
    parser.add_argument("--focal_loss_medical", action="store_true", default=False)
    parser.add_argument("--focal_loss_gamma", default=4.0, type=int)
    parser.add_argument("--dice_weight", default=0.4, type=int)
